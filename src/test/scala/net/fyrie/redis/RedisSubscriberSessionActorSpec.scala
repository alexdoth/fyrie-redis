...
class RedisSubscriberSessionActorSpec extends SetakWordSpec {
  ...
  "RedisSubscriberSession" when {
    "connecting to a server" should {
      ...
      "shutdown if the server refuses the connection and auto-reconnect is disabled" in offline {
        testMessagePatternEnvelop(anyActorRef, worker, { case IO.Closed(handle, cause) ⇒ })
        testMessageEnvelop(worker, subscriberSession, Disconnect)
      ...
      }(RedisClientConfig(autoReconnect = false))
    }

    // NOTE: all tests below require a running Redis server
    "connected to a server" should {

      val channel1 = "Test Channel 1"
      val channel2 = "Test Channel 2"
      val pchannel = "Test*"

      def online(body: ⇒ Unit) {
        ...
        try {
          ...
          case e: Exception ⇒ fail("Unable to connect to server. Is redis server running?")
        }

        body
      }

      "subscribe and unsubscribe to a channel and a pattern" in online {
        subscribe(channel1, channel2)
        psubscribe(pchannel)
        unsubscribe(channel1, channel2)
        punsubscribe(pchannel)
      }

...
class IOManager(bufferSize: Int = 8192) extends Actor {
  ...
  override def postStop {
    worker(Shutdown)
    // wait for the worker to really shutdown
    try {
      worker.join()
    } catch {
      case e: InterruptedException ⇒ EventHandler.warning(this, "IOWorker interrupted")
    }
  }
...
private[akka] class IOWorker(ioManager: ActorRef, val bufferSize: Int) {
  ...
  // added so that the IOManager stops atomically
      ...
      case Some(channel) ⇒
        ...
        try {
          ...
          case e: ActorInitializationException ⇒
            EventHandler info (this, "IO.Handle's owner not running")
        }
      case None ⇒
    }
  }

...
private[redis] final class RedisClientSession(
  ioManager: ActorRef,
  host: String,
  port: Int,
  config: RedisClientConfig) extends Actor {
  ...
  override def preStart = {
    ...
    // moved this outside so that we can more easily test the actor system
...
private[redis] final class RedisClientWorker(ioManager: ActorRef,
                                             host: String,
                                             port: Int,

...
import akka.logger.LogActorFactory.actorOf
...
object RedisClient {
  def apply(host: String = "localhost",
            ...
            ioManager: ActorRef = actorOf(new IOManager()).start) =
    new RedisClient(host, port, config, ioManager)

  def subscriber(listener: ActorRef)(host: String = "localhost",
                                     port: Int = 6379,
                                     config: RedisClientConfig = RedisClientConfig(),
                                     ioManager: ActorRef = actorOf(new IOManager()).start): ActorRef =
    actorOf(new RedisSubscriberSession(listener)(ioManager, host, port, config)).start
}

final class RedisClient(val host: String = "localhost",
                        val port: Int = 6379,
                        val config: RedisClientConfig = RedisClientConfig(),
                        val ioManager: ActorRef = actorOf(new IOManager()).start)
  extends RedisClientAsync(config) {
...
sealed abstract class RedisClientAsync(config: RedisClientConfig)
  extends Commands[Future] with ConfigurableRedisClient {
...
sealed abstract class RedisClientSync(config: RedisClientConfig)
  extends Commands[({ type λ[α] = α })#λ] with ConfigurableRedisClient {
  implicit private val timeout = config.timeout
...
sealed abstract class RedisClientQuiet(config: RedisClientConfig)
  extends Commands[({ type λ[_] = Unit })#λ] with ConfigurableRedisClient {
  final private[redis] def send(in: List[ByteString]) = actor ! Request(format(in))

  val quiet: RedisClientQuiet = this
}

sealed abstract class RedisClientMulti(config: RedisClientConfig)
  extends Commands[({ type λ[α] = Queued[Future[α]] })#λ]()(ResultFunctor.multi) {
  implicit private val timeout = config.timeout
...
import commands._
private[redis] sealed abstract class Commands[Result[_]](implicit rf: ResultFunctor[Result])
  extends Keys[Result]
  with Servers[Result]
  with Strings[Result]
  with Lists[Result]
  with Sets[Result]
  with SortedSets[Result]
  with Hashes[Result]
  with PubSub[Result]
  with Scripts[Result] {
